...
이 케이스는 UART serial input이 실제로 FIFO에 쌓이고, 이후 pop 순서가 깨지지 않는지를 보는 통합 검증 사례입니다. `asdf`의 UART RX 스타일을 참고하되, FIFO는 현재 포트폴리오의 `sync_fifo` 자산을 재사용하도록 구성했습니다.
...
즉, 이 환경은 “UART RX가 동작한다”를 넘어서 “UART RX 결과가 FIFO를 지나도 순서가 깨지지 않는가”를 보여주도록 설계했습니다.
...
| `balanced` | 송신과 pop을 거의 1:1로 유지 | steady-state에서 serial RX와 FIFO pop이 균형 있게 이어졌음을 보여줍니다. |
...
즉, 이 환경의 핵심 타이밍은 "UART RX 수신 완료 -> FIFO push -> pop valid/data 정렬"입니다.
...
- UART serial protocol과 FIFO ordering 검증을 한 번에 보여주는 통합 사례
- 기존 `sync_fifo` 자산을 실제 시스템 경로에 재사용
- 타이밍 misalignment를 실검증 중 수정해 clean pass 확보
- 단일 블록 검증에서 끝나지 않고, protocol + buffering 통합 검증으로 확장했다는 점을 어필 가능
