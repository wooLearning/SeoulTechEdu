...
def build_start_here(root: Path, paths: ReportPaths) -> str:
    return "\n".join(
        [
            "# START HERE",
            "",
            "이 폴더는 `result_portfolio`의 Python 시각화 포트폴리오 최종본입니다.",
            "",
            "## 가장 먼저 볼 파일",
            "",
            ...
            "## 추천 확인 순서",
            "",
            "1. HTML 보고서로 전체 톤과 시각화를 확인",
            "2. dashboard PNG와 heatmap PNG로 핵심 그래프 확인",
            "3. overview/module_reports 문서로 시나리오, coverage, timing 설명 확인",
            "",
            "## 참고",
            "",
