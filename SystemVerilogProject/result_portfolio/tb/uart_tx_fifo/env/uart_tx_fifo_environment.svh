class uart_tx_fifo_environment;
  virtual uart_tx_fifo_if vif;
  uart_tx_fifo_generator  gen;
  uart_tx_fifo_driver     drv;
  uart_tx_fifo_monitor    mon;
  uart_tx_fifo_scoreboard scb;
  ...
  function new(virtual uart_tx_fifo_if vif);
    this.vif = vif;
    gen2drv_mbox = new();
    exp_mbox = new();
    mon2scb_mbox = new();
  ...
  task run(int run_count);
    ...
    @(scb_done_ev);
    disable fork;
    $display("[UART-TXF][ENV] run finished");
  endtask
endclass
