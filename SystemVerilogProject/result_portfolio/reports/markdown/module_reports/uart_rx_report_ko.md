...
이 케이스는 `asdf` 경로의 UART RX 검증 스타일을 참고해 `veri1` 구조로 정리한 UART showcase입니다. 폴더 구조와 패키징은 현재 포트폴리오 스타일을 유지하고, serial protocol 자극 방식은 `asdf`의 task-based UART driver 패턴을 채택했습니다.
...
즉, UART RX는 “정상 수신”만 보는 게 아니라 “프레임 오류에서 valid를 올리지 않는가”까지 같이 검증합니다.
...
- scenario bin
  ...
  - invalid stop
- cross
  - data pattern x scenario

이 설계를 택한 이유는 UART RX에서 중요한 포인트가 단순 랜덤이 아니라, sanity pattern과 framing 오류를 함께 보는 데 있기 때문입니다.
...
- printable ASCII
  - 사람이 읽기 쉬운 일반 payload 영역이 실제로 수신 경로를 통과하는지 설명하기 좋습니다.
- other
  - sanity pattern 외 일반 binary payload가 충분히 섞였는지 보여줍니다.
...
즉, UART RX coverage는 "정상 프레임 payload 분포"는 covergroup으로, "비정상 프레임 억제"는 scoreboard negative check로 역할을 나눠 설명하는 구조입니다.
...
즉, RX 경로는 `serial task stimulus -> oValid pulse monitor -> positive/negative scoreboard`로 흘러갑니다.
