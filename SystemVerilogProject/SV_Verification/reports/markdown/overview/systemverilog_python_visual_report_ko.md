...
- 현재 Python 시각화는 FIFO 계열 showcase를 중심으로 구성돼 있고, UART 확장 케이스는 모듈별 보고서와 Vivado 로그로 보강했습니다.

## UART 확장 보고서 동선

UART 추가분은 아래 문서에서 자세히 확인할 수 있습니다.
...
즉, 이 Python 리포트는 FIFO 계열의 CSV/그래프 해석을 담당하고, UART 계열은 protocol/bridge 관점의 상세 문서가 설명을 이어받는 구조입니다.
...
- `cx_scenario_*` cross: 특정 시나리오에서 특정 flag/path/request mix가 실제로 함께 나왔는지 연결합니다.
- CSV 기반 `trace` 분석: covergroup 퍼센트와 별개로 depth 분포와 시간 흐름을 다시 시각화해 coverage 해석 근거를 보강합니다.

참고로 XSim 로그의 `coverage_pct`는 현재 0.00%로 남을 수 있습니다. 이 값은 리포트 포맷 한계에 가깝고, 실제 포트폴리오 설명은 covergroup bin 의도와 TB가 남긴 summary/scenario/trace CSV를 함께 읽는 방식으로 구성했습니다.
