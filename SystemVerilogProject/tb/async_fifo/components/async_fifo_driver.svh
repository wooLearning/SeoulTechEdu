// Drives async_fifo write/read requests on separate clocks.
class async_fifo_driver;
  ...
  task run();
    assert_vif_valid: assert (vif != null)
      else $fatal(1, "vif is null in async_fifo_driver");

    forever begin
      gen2drv_mbox.get(tr);
      assert_tr_valid: assert (tr != null)
        else $fatal(1, "Received null transaction in async_fifo_driver");
