...
이 문서는 `veri1` 프로젝트를 포트폴리오 용도로 재정리한 결과를 자세히 설명하기 위한 보고서입니다.

처음 의도는 다음과 같았습니다.

- 기존에 학습/실습 형태로 작성한 검증 코드를 포트폴리오에서 읽기 쉬운 구조로 정리
- 단순 RTL 구현이 아니라 SystemVerilog 검증 역량까지 함께 어필
...
즉, 루트에 원래 있던 `src/`, `tb/`는 참고용으로만 읽었고 직접 수정 대상은 `veri1` 내부로 제한했습니다.
...
따라서 이번 정리에서는 아래 판단을 기준으로 진행했습니다.

- “단일 대형 파일을 정확히 복원”하는 것보다는
- 현재 가장 안정적으로 검증이 통과하는 분리 구조를 기준으로
- 포트폴리오 관점에서 더 읽기 쉽고 설명하기 좋은 형태로 업그레이드
...
즉, “UVM을 썼다”라기보다는 “UVM 스타일의 역할 분리를 직접 구현해서 검증 환경을 구성했다”는 쪽이 더 정확한 설명입니다.

## 5. 코드 구조 정리 내용

### 5-1. 폴더 구조

showcase TB는 다음 구조를 유지하도록 정리했습니다.
...
showcase 코드에는 아래 원칙으로 주석을 정리했습니다.
...
이번 포트폴리오에서는 아래 항목을 문서에서 바로 찾을 수 있게 정리했습니다.
...
즉, overview 문서는 전체 구조와 의도를 설명하고, 모듈별 상세 보고서는 class/coverage/assertion/timing을 깊게 설명하며, Python 시각화 보고서는 시나리오별 수치와 그래프 해석을 담당하도록 역할을 나눴습니다.

## 5-5. UART 확장 사례 정리

FIFO showcase를 정리한 뒤, 포트폴리오 메시지를 더 넓히기 위해 UART 계열 통합 검증도 추가했습니다.
...
즉, 단일 FIFO 블록 검증에서 끝나지 않고, protocol + buffer 조합까지 확장 가능한 구조라는 점을 보여주도록 정리했습니다.

## 6. Async FIFO 검증 구조 설명

메인 showcase는 `fifo`입니다.
...
즉, “신호가 올라갔다”와 “DUT가 실제로 수락했다”를 구분하는 검증 구조입니다.
...
- write path coverage
  - idle, accepted, blocked가 모두 발생했는지 확인
- read path coverage
  - idle, accepted, blocked가 모두 발생했는지 확인
- flag state coverage
...
즉, 이 coverage는 “몇 퍼센트냐”보다 “의도한 경로가 실제로 다 나왔느냐”를 설명하기 위한 도구입니다.

Async FIFO showcase의 최신 시나리오별 집계는 다음과 같습니다.

| 시나리오 | 샘플 | WR Acc | RD Acc | WR Block | RD Block | 해석 |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| `fill_burst` | 229 | 23 | 8 | 19 | 1 | fill 구간에서 write accepted와 write blocked가 함께 나와 full 근처 압박이 실제로 걸렸음을 보여줍니다. |
...
동기 FIFO는 비동기 FIFO보다 구조는 단순하지만, 다음 역량을 깔끔하게 보여줍니다.
...
즉, async FIFO가 무겁고 강한 사례라면, sync FIFO는 더 읽기 쉬운 보조 showcase 역할을 합니다.
...
sync FIFO는 단일 클록이기 때문에 async FIFO보다 단순해 보일 수 있지만, 시나리오 설계는 오히려 정책 검증을 더 명확하게 보여줍니다.
...
즉, sync FIFO에서는 “동시 read/write 허용 조건을 scoreboard가 제대로 따라가는가”가 가장 중요한 시나리오 메시지입니다.
...
즉, 현재 최종 구조는 “문법만 clocking block”이 아니라, drive / pre-sample / post-sample을 분리한 clocking-block-based verification structure라고 설명할 수 있습니다.
...
즉, 두 showcase 모두 scoreboard 기준으로 clean pass 상태입니다.
...
- showcase와 reference 범위 구분
- 실행 증빙과 문서 경로 정리
- FPGA_AUTO 기반 반복 가능한 실행 흐름 확보

## 12. 남겨둔 참고 자산

이번 `result_portfolio` 패키지는 GitHub 업로드용 최종본이라, showcase에 직접 기여하는 코드와 보고서만 남겼습니다.

즉, 원본 프로젝트에 있던 학습형/reference 자산 전체를 그대로 옮긴 것이 아니라 아래 기준으로 선별한 것입니다.
