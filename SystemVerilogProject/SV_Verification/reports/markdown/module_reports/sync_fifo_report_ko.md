...
이 모듈은 async FIFO보다 구조는 단순하지만, queue 기반 reference model과 full/empty flag 검증을 비교적 읽기 쉬운 형태로 보여주는 서브 showcase입니다.
...
즉, 단순 FIFO처럼 보이지만 동시 read/write와 full/empty update 타이밍이 검증 포인트가 됩니다.
...
- `oFull`
- `oEmpty`
- `cp_wr x cp_rd x cp_full x cp_empty` cross
...
즉, sync FIFO는 단일 clock DUT이지만 시나리오를 분리해서 “정상 처리”, “동시 처리”, “경계 압박”을 각각 눈에 보이게 설계했습니다.
...
즉, sync FIFO coverage는 “단일 clock라서 단순하다”가 아니라, same-cycle read/write와 flag state를 교차해서 봤다는 점이 핵심입니다.
...
| `balanced_stream` | 63 | 45 | 42 | 0 | 6 | steady-state stream과 경미한 empty pressure를 함께 보여줍니다. |
...
즉, sync FIFO는 async처럼 pre-edge flag를 따로 transaction에 담지는 않지만, “drive phase”와 “observe phase”를 clocking block으로 분리해 request와 결과를 안정적으로 정렬합니다.
