...
비동기 FIFO를 대상으로 한 메인 검증 사례입니다. 단일 클록 설계보다 검증 항목이 많고, CDC를 고려한 검증 구조를 확인하기에 적합합니다.
...
top TB는 전체 orchestration을 담당하고, 실제 검증 로직은 class 기반 환경으로 위임합니다.
...
- scenario, domain, write/read path, flag 상태 covergroup 수집
- `option.per_instance = 1;`, `type_option.merge_instances = 1;` 설정
- scoreboard에서 `cg_fifo.get_inst_coverage()` 출력

의미:

- coverage는 “어떤 domain에서 어떤 path와 flag가 관측됐는가”를 수집합니다.
...
scoreboard는 enable만으로 판단하지 않고, 직전 full/empty 상태를 기반으로 수락된 transaction만 reference model에 반영합니다.
...
phase bias를 둔 constrained-random 시나리오입니다.
...
coverage는 scenario와 boundary state의 관측 여부를 함께 정리합니다.

최신 실행 기준 시나리오별 coverage 집계는 아래와 같습니다.

| 시나리오 | 샘플 | WR Acc | RD Acc | WR Block | RD Block | 커버리지 해석 |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| `fill_burst` | 229 | 23 | 8 | 19 | 1 | fill/full pressure와 write blocked path 관측을 확인합니다. |
...
현재 `fifo` 환경은 `clocking block`을 중심으로 drive/sample discipline을 구성합니다.
...
clocking block에 타이밍 의도를 담아 acceptance 판정과 결과 관측을 분리한 구조입니다.
...
- coverage는 Vivado native `get_inst_coverage()` 기준으로 집계함
