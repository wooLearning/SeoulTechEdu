...
이 케이스는 기존 `tb/fifo/` 구조를 그대로 따라가는 것이 아니라, `src/async_fifo.sv`의 reset polarity와 인터페이스 정의를 반영해 별도 `tb/async_fifo/` 환경으로 분리한 사례입니다.
...
즉, FSM 기반 모듈이 아니라 pointer/synchronizer 기반 제어 구조이며, 검증 포인트도 상태 천이보다는 clock-domain crossing 이후 flag 일관성과 data ordering에 맞춰집니다.
...
- flag state coverage
  - normal / full / empty
- scenario x domain cross
- scenario x flag cross

보완 설명:

- XSim 로그의 기본 coverage 출력은 0.00%로 남지만, 클래스 내부 `get_coverage()`는 hit 기반 커스텀 coverage 비율을 계산합니다.
- 따라서 포트폴리오 해석에서는 시나리오 완주율과 response diversity까지 같이 보는 편이 더 설득력 있습니다.
...
- UVM factory나 phase mechanism은 쓰지 않았지만, 역할 분리와 top-level orchestration은 UVM-inspired 구조를 잘 보여줍니다.
...
즉, 현재 구조는 “기본 interface-level SVA + scoreboard 중심 데이터 무결성 검증” 조합입니다.
...
- cross coverage
  - scenario와 domain, scenario와 flag가 어떤 조합으로 나타났는지 확인

즉, coverage의 핵심은 “FIFO가 동작했다”가 아니라, “의도한 모든 시나리오와 경계 상태가 실제로 관측되었다”는 점을 정량화하는 것입니다.
...
이 구조의 장점은 “수동 `#1` 지연”이 아니라 “clocking block 자체에 타이밍 의도”를 담았다는 점입니다. 즉, driver는 입력 안정화, monitor는 판정 근거와 결과 분리 수집, scoreboard는 accepted transaction 추적으로 역할이 명확히 구분됩니다.
...
- 포트폴리오에서 “기존 showcase 외에 추가 RTL도 직접 구조화해 검증했다”는 점을 어필하기 좋음
