...
FIFO에 적재된 byte가 UART TX로 순서대로 전달되는지를 다루는 통합 사례입니다. serial input 경로가 아니라 `buffered transmit path` 검증에 초점을 둡니다.
...
이 시나리오 구성은 serial output waveform 전체보다 FIFO 적재 후 UART로 빠져나가는 순서를 관찰하기 위한 설정입니다.
...
| `balanced_enqueue` | enqueue와 launch가 번갈아 일어나는 steady-state | `oBusy`와 launch handoff의 연속성을 확인합니다. |
...
이 케이스의 핵심은 serial waveform 자체보다 FIFO dequeue timing과 UART launch timing의 정렬입니다.
