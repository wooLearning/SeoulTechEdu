class uart_async_fifo_driver;
  ...
  task preset();
    ...
    vif.iRx  <= 1'b1;
    vif.iPop <= 1'b0;
    repeat (6) @(posedge vif.iWrClk);
    vif.iRst <= 1'b0;
    repeat (4) @(posedge vif.iWrClk);
    $display("[UART-AF][DRV] reset released");
  endtask
  ...
  task automatic pop_once_if_available();
    bit ready;
    ready = 1'b0;
    if (vif.oEmpty) wait_fifo_has_data(4000, ready);
    else ready = 1'b1;
    if (ready) begin
      @(negedge vif.iRdClk);
      vif.iPop <= 1'b1;
  ...
  task run(int run_count);
    for (int i = 0; i < run_count; i++) begin
      ...
      $display("[UART-AF][DRV] sent byte=%0h scenario=%s pop_after=%0d t=%0t",
    ...
    $display("[UART-AF][DRV] drained remaining async FIFO content");
  endtask
endclass
