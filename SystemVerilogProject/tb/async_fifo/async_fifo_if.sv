...
Name: async_fifo_if
Target: async_fifo
Role: Interface for connecting tb_async_fifo and async_fifo DUT
Scenario:
  - Provides grouped dual-clock FIFO request/response signals
CheckPoint:
...
interface async_fifo_if;
  ...
  property p_flags_not_both_high_wr;
    @(posedge iWrClk) disable iff (iRst) !(oFull && oEmpty);
  endproperty

  property p_flags_not_both_high_rd;
    @(posedge iRdClk) disable iff (iRst) !(oFull && oEmpty);
  endproperty

  property p_reset_drives_empty_wr;
    @(posedge iWrClk) iRst |=> (oEmpty && !oFull);
  endproperty

  property p_reset_drives_empty_rd;
    @(posedge iRdClk) iRst |=> (oEmpty && !oFull);
  endproperty

  a_flags_not_both_high_wr: assert property (p_flags_not_both_high_wr)
    else $error("[ASSERT][async_fifo_if] write-domain flags cannot be high together");

  a_flags_not_both_high_rd: assert property (p_flags_not_both_high_rd)
    else $error("[ASSERT][async_fifo_if] read-domain flags cannot be high together");

  a_reset_drives_empty_wr: assert property (p_reset_drives_empty_wr)
    else $error("[ASSERT][async_fifo_if] reset should drive empty/high and full/low on write clock");

  a_reset_drives_empty_rd: assert property (p_reset_drives_empty_rd)
    else $error("[ASSERT][async_fifo_if] reset should drive empty/high and full/low on read clock");

  modport dut (
    input  iWrClk, iRdClk, iRst, iWrEn, iRdEn, iWData,
