...
이 케이스는 UART RX 결과를 비동기 FIFO로 넘긴 뒤, 독립된 read clock 도메인에서 순서대로 꺼낼 수 있는지를 검증하는 통합 사례입니다. 즉, protocol 검증과 CDC buffering 검증을 한 경로에 묶은 showcase입니다.
...
즉, 이 환경은 단순히 "UART RX가 되느냐"가 아니라, "UART RX 결과가 async FIFO를 지나 다른 clock domain으로 넘어가도 ordering이 보존되느냐"를 검증하는 구조입니다.
...
| `balanced_async` | write/read가 거의 균형 | dual-clock steady-state에서도 ordering이 유지됨을 보여줍니다. |
| `drain_async` | read side 우세 구간 | empty 근처와 pop pressure가 실제로 만들어졌음을 설명합니다. |

이 케이스의 coverage는 단순히 phase hit 여부가 아니라, dual-clock 환경에서도 scenario intent와 flag behavior가 함께 관측됐는지를 보여주는 역할입니다.
...
즉, `serial RX in write domain -> async FIFO crossing -> ordered pop in read domain`이 이 케이스의 핵심 timing story입니다.
...
- protocol block과 buffering block을 결합한 시스템성 검증 사례로 어필 가능
