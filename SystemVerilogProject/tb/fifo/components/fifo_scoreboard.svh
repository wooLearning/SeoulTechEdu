// Reference model + checks for async FIFO behavior (coverage delegated to fifo_coverage).
class fifo_scoreboard;
  ...
  // Queue-based reference model for accepted FIFO contents.
  logic [7:0] rModelQ[$];
  ...
  task automatic write_trace(
    ...
    if (rTraceFd != 0) begin
      $fwrite(
        rTraceFd,
        "async_fifo,%0d,%0t,%s,%0d,%s,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d\n",
        rSampleCnt,
        $time,
        domain_name,
        tr.scenario_id,
  ...
  task run(int run_count);
    setup_csv();
    // Driver emits exactly one read-domain request window per generated transaction.
    // Write-domain monitor can observe extra idle samples because write clock is faster.
    while (rRdTickCnt < run_count) begin
      ...
      if (tr.isWrSample) begin
        // Only accepted writes update the reference model.
      ...
      if (tr.isRdSample) begin
        rRdTickCnt++;
        // Compare read data only when the read request was actually accepted.
    ...
    $display("[SCB][COVERAGE] functional_coverage=%0.2f%%", cov.cg_fifo.get_inst_coverage());
    if (rSummaryFd != 0) begin
      $fwrite(
        rSummaryFd,
        "async_fifo,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0.2f,PASS\n",
        rSampleCnt,
        rRdTickCnt,
        rPassCnt,
        rFailCnt,
    ...
    foreach (rScenarioSampleCnt[idx]) begin
      ...
      if (rScenarioFd != 0) begin
        $fwrite(
          rScenarioFd,
          "async_fifo,%0d,%s,%0d,%0d,%0d,%0d,%0d\n",
          idx,
          fifo_transaction::scenario_name_by_id(idx),
          rScenarioSampleCnt[idx],
          rScenarioWrAcceptCnt[idx],
    ...
    $display("[SCB][PASS] Async FIFO scoreboard completed without mismatches");
