...
<body>
  <main>
    <section class="hero">
      ...
      <p class="note">참고: XSim 로그의 기본 `functional_coverage` 값은 현재 0.00%로 남아 있어, 본 리포트에서는 별도로 `scenario completion`, `response diversity`, `portfolio score`를 계산해 해석 지표로 함께 제시합니다.</p>
    </section>
...
def build_markdown_report(project_root: Path, module_metrics: list[ModuleMetric], analytics_dir: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "- XSim 기본 coverage 수치가 0.00%로 남는 한계를 보완하기 위해, 본 보고서에서는 시나리오 완주율과 accepted/blocked/flag 다양성을 합친 `portfolio score`를 같이 제시합니다.",
            "",
            "## 재생성 방법",
            "",
...
def build_console_report(module_metrics: list[ModuleMetric], scenario_metrics: list[ScenarioMetric]) -> str:
    ...
    lines.extend(
        [
            ...
            "- XSim built-in functional coverage is 0.00%, so scenario completion and response diversity are shown together",
